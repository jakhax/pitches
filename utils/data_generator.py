...
from pitches.app import db
from pitches.models import User, Topic, TopicGroup, Comment, Message, PollAnswer, PollVote
...
def generate_fake_topic_groups(count=10, parent_group_id=0):
    seed()
    user_count = db.session.query(func.count(User.id)).scalar()
    fake_topic_groups_titles=['pickup lines', 'interview pitch' , 'product pitch', 'promotion pitch.']
    for i in fake_topic_groups_titles:
        u = User.query.offset(randint(0, user_count - 1)).first()
        g = TopicGroup(title=i,
